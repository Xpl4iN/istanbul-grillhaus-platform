...
export async function POST(req: Request) {
    try {
        ...
        // The field name in the schema is 'score', but some clients might still send 'rating'
        const finalScore = (typeof score === 'number' ? score : (body as any).rating) as number;

        if (typeof finalScore !== "number" || finalScore < 1 || finalScore > 5 || !Number.isInteger(finalScore)) {
